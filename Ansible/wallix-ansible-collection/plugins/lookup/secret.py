...
- name: Retrieve password
  debug:
    msg: "{{ lookup('wallix.pam_secret_action.secret', 'admin@local@prod-db-01') }}"
'''
...
class LookupModule(LookupBase):
    ...
    def run(self, terms, variables=None, **kwargs):
        ...
        if not api_key and not (username and password):
             raise AnsibleError("Either WALLIX_API_KEY or username/password are required.")
        ...
        for term in terms:
            ...
            try:
                response = requests.get(url, headers=headers, auth=auth, verify=validate_certs)
                ...
                if response.status_code == 200:
                    ...
                    raise AnsibleError(f"Wallix API Error {response.status_code}: {response.text}")
