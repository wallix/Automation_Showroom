...
- **Retrieval Task:** The task that calls `wallix.pam.secret` has logging disabled to prevent the retrieved password or SSH key from appearing in the job logs.
...
- **Masking:** Variables are marked as **Masked** so they are redacted in job logs.
  - _Note:_ GitLab has strict requirements for masked variables (min 8 chars, specific characters only). If a variable cannot be masked (e.g., contains `$`), it must be handled with extra care (like `no_log: true` in Ansible).
